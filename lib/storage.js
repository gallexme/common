...
class Queue {
    static fetch(name, cb) {
      RedisConnection.rpop(name, (err,task)=> {
        ...
        if(task == null) {
          setTimeout(Queue.fetch,50,name,cb);
          return;
        }
...
function queueConnect() {
    ...
    d.on('remote', remote => {
        ...
        if (!RedisConnection) {
          ...
          Object.assign(exports.queue, {
              fetch: resetInterceptor(q.nbind(Queue.fetch, Queue)),
              add: resetInterceptor(q.nbind(Queue.add, Queue)),
              addMulti: resetInterceptor(q.nbind(Queue.addMulti, Queue)),
              markDone: resetInterceptor(q.nbind(Queue.markDone, Queue)),
              whenAllDone:resetInterceptor(q.nbind(Queue.whenAllDone, Queue)),
              reset:resetInterceptor(q.nbind(Queue.reset, Queue))
          });
          let bak = RedisConnection.set;
          RedisConnection.set = function () {
            console.log("SET",arguments)
            return bak.apply(arguments);
          }
          Object.assign(exports.env, {
            get: resetInterceptor(q.nbind(RedisConnection.get, RedisConnection)),
            mget: resetInterceptor(q.nbind(RedisConnection.mget, RedisConnection)),
            set: resetInterceptor(q.nbind(RedisConnection.set, RedisConnection)),
            setex: resetInterceptor(q.nbind(RedisConnection.setex, RedisConnection)),
            expire: resetInterceptor(q.nbind(RedisConnection.expire, RedisConnection)),
            ttl: resetInterceptor(q.nbind(RedisConnection.ttl, RedisConnection)),
            del: resetInterceptor(q.nbind(RedisConnection.del, RedisConnection)),
            hmset:resetInterceptor(q.nbind(RedisConnection.hmset, RedisConnection)),
          });
            Object.assign(exports.pubsub, {
                publish: resetInterceptor(q.nbind(RedisConnection.publish, RedisConnection)),
                subscribe:resetInterceptor(q.nbind(SubscriberRedisConnection.subscribe, SubscriberRedisConnection)),
            });
